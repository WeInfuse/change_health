  ...
  describe 'eligibility data' do
    ...
    describe 'benefit' do
      ...
      it '#employee_and_child?' do
        assert_predicate(klazz.new(coverageLevelCode: klazz::EMPLOYEE_AND_CHILD), :employee_and_child?)
      ...
      it '#visit?' do
        assert_predicate(klazz.new(timeQualifierCode: klazz::VISIT), :visit?)
        assert_equal(false, klazz.new.visit?)
      end

      it '#remaining?' do
        assert_predicate(klazz.new(timeQualifierCode: klazz::REMAINING), :remaining?)
        assert_equal(false, klazz.new.remaining?)
      end

      it '#year?' do
        assert_predicate(klazz.new(timeQualifierCode: klazz::YEAR), :year?)
        assert_equal(false, klazz.new.year?)
      end

      it '#copayment?' do
        assert_predicate(klazz.new(code: klazz::COPAYMENT), :copayment?)
        assert_equal(false, klazz.new.copayment?)
      end

      it '#coinsurance?' do
        assert_predicate(klazz.new(code: klazz::COINSURANCE), :coinsurance?)
        assert_equal(false, klazz.new.coinsurance?)
      end

      it '#non_covered?' do
        assert_predicate(klazz.new(code: klazz::NON_COVERED), :non_covered?)
        assert_equal(false, klazz.new.non_covered?)
      end

      it '#out_of_pocket?' do
        assert_predicate(klazz.new(code: klazz::OUT_OF_POCKET), :out_of_pocket?)
        assert_equal(false, klazz.new.out_of_pocket?)
      end

      describe '#in_plan_network?' do
        it 'non medicare' do
          assert_predicate(klazz.new(inPlanNetworkIndicatorCode: 'Y'), :in_plan_network?)
          assert_predicate(klazz.new(inPlanNetworkIndicatorCode: 'Y'), :in_plan?)
          assert_predicate(klazz.new(inPlanNetworkIndicatorCode: 'Y'), :in_network?)
          assert_equal(false, klazz.new.in_plan_network?)
        end

        it 'medicare' do
          assert_predicate(klazz.new(insuranceTypeCode: 'MA'), :in_plan_network?)
      ...
      describe '#services' do
        it 'returns readable service type and code' do
          assert_equal([['98', 'Professional (Physician) Visit - Office']],
                       klazz.new(serviceTypeCodes: ['98'],
                                 serviceTypes: ['Professional (Physician) Visit - Office']).services)
        end
        ...
        it 'handles no type codes' do
          assert_empty(klazz.new(serviceTypes: ['cat']).services)
        end

        it 'handles no types' do
          assert_empty(klazz.new.services)
        end
      end
        ...
        describe 'with no info' do
          ...
          describe 'descriptions' do
            it 'is empty array' do
              assert_empty(benefit.descriptions)
            end
          end
        end
        ...
        describe 'with info' do
          ...
          describe 'descriptions' do
            it 'is array with only description fields' do
              assert_equal(%w[cat dog], benefit.descriptions)
            end
          end
        end
      end
        ...
        describe 'helpers' do
          ...
          describe 'plan range' do
            ...
            describe 'has range' do
              ...
              it 'empty array' do
                assert_equal(%w[20030111 99991231], benefit.plan_date_range)
            ...
            describe 'is missing' do
              ...
              it 'has no data' do
                assert_empty(benefit.plan_date_range)
