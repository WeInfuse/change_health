...
class Report277ClaimTest < Minitest::Test
  ...
  describe 'info_claim_statuses' do
    ...
    it 'latest status category codes' do
      assert_equal info_claim_status_new.info_statuses.map(&:status_category_code),
                   multi_status_report_claim.latest_status_category_codes
    end
    ...
    it 'invalid claim status' do
      info_claim_status_bad = ChangeHealth::Response::Claim::Report277InfoClaimStatus.new(
        status_information_effective_date: 'not a date!'
      )
